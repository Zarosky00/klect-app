...
| 0018 | `pulse_first_class` | Applied 2026-07-26, smoke-tested under JWT impersonation, advisors 0 ERROR. `post_media` (mirrors `item_media`), `create_post` RPC (the only insert path for posts — body/quote/reply/media, snake_case errors, returns the pulse envelope), `pulse_feed` rewrite (LIMIT-after-`jsonb_agg` bug fixed, `p_mode` following\|foryou with taste-ranked foryou, server-embedded `target` payloads so repost-of-post renders, real `viewer_reposted`), and the INSERT counter-column audit (posts table INSERT revoked outright — **breaks the web composer's direct insert until B5w**; collections/subcollections/items/comments re-granted column lists without counters; `posts_check` dropped in favour of RPC validation). Dry-run validated against the live schema in a rolled-back transaction 2026-07-26 |
| 0019 | `slug_autogen` | **P0 fix**: `slugify()` + BEFORE INSERT triggers deriving `collections`/`subcollections` slugs from `name` (unique-suffixed per scope). Clients always inserted slugless and nothing server-side filled it — onboarding shelf creation failed with a not-null violation since 0001. Applied + smoke-tested 2026-07-27 |
| 0020 | `read_policy_owner_fastpath` | **P0 fix**: inline `auth.uid() = user_id` fast-path prepended to `subcollections_read`/`items_read` — the bare `can_view_entity(id)` self-lookup cannot see a row mid-INSERT, so every client `.insert().select()` (RETURNING) on subcollections failed RLS since 0008. Applied + smoke-tested end-to-end (slugless collection→subcollection→item with RETURNING) 2026-07-27 |
| 0021 | `threads_and_feed_fixes` | Applied 2026-07-27, smoke-tested under JWT impersonation (7 rolled-back blocks: thread top/new/bad_sort/has_more/viewer state, user_posts envelopes, composite-cursor twin paging + extra-row has_more, foryou 90d ladder rung, comment toggle_save/toggle_repost `{active:true,count:1}`, `replies_use_comments`, search_all posts section + anon private-account exclusion), advisors 0 ERROR (+4 accepted definer-executable WARNs: get_post_thread/user_posts × anon+authenticated). ROUND3 C0. Comments gain `save_count`/`repost_count` (backfilled) with the `bump_entity_counter`/`entity_counter` comment branches (fixes the 0018 comment-repost feed-poisoning risk; `pulse_target_payload` comment branch now carries `parent_type`/`parent_id`); `get_post_thread` (post envelope + stats + paged comments with batched viewer state; errors `bad_sort`/`post_not_found`); `user_posts` (profile Posts tab, pulse envelopes); `pulse_feed` composite keyset cursor `(created_at, id)` via `p_before_id` + extra-row has_more (returns up to p_limit+1; new additive `cursor_id` envelope key) + For-you entity reposts + 30d→90d→all-time window ladder; `create_post` rejects `p_reply_to` (`replies_use_comments`); `search_all` gains a `posts` section (tsv+GIN guarded — column already lived in 0003's schema) with the people section gated by `can_see_owner` |
