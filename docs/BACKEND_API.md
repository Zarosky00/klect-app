...
| `create_post` | `p_body?`, `p_kind='post'`, `p_entity_type?`+`p_entity_id?` (entity share, or quote when `'post'`), `p_media jsonb?` (≤4 descriptors under `{uid}/posts/{draft}/…` in the `media` bucket), ~~`p_reply_to`~~ (0021: raises `replies_use_comments` — comments are the discussion primitive) | `jsonb` — the new post's full pulse envelope. **The only insert path for posts** (0018 revoked direct INSERT). Stable errors: `body_or_attachment_required`, `bad_target`, `entity_not_found`, `blocked`, `bad_media`, `too_many_media`, `media_not_yours`, `body_too_long`, `replies_use_comments` |
| `get_post_thread` | `p_post`, `p_limit=30`, `p_before?`, `p_sort='top'\|'new'` | `jsonb` — the X thread payload (0021): `{post: <envelope>, stats, comments:[… incl. save/repost counts + viewer state], has_more}`. Anon-callable (SEO). Errors: `bad_sort`, `post_not_found` |
| `user_posts` | `p_user`, `p_limit=25`, `p_before?`, `p_before_id?` | `jsonb[]` — profile Posts tab (0021): author's posts/quotes + entity reposts, pulse envelopes, extra-row has_more |

**Pagination contract (0021):** `pulse_feed`/`user_posts` return up to `p_limit+1` rows — `length > p_limit` ⇒ more pages. Every envelope carries `cursor_id`; page with `p_before = sort_at` **and** `p_before_id = cursor_id` of the last rendered row (fixes same-timestamp skips). Comments carry `save_count`/`repost_count` (0021) and all `toggle_*` RPCs accept `entity_type='comment'`.
